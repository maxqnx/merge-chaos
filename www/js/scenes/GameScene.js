...
class GameScene extends Phaser.Scene {
  ...
  drawBackground() {
    ...
    // Neon Cyberpunk deep dark gradient
    bg.fillGradientStyle(0x030318, 0x030318, 0x010114, 0x010114, 1);
    bg.fillRect(0, 0, W, H);
    ...
    // Lane dividers — electric violet
    const lanes = this.add.graphics().setDepth(1);
    lanes.lineStyle(1, 0x6600ff, 0.18);
    ...
    for (let row = 0; row < 7; row++) {
      for (let col = 0; col < 3; col++) {
        ...
        gridGfx.lineStyle(1, 0x00f0ff, 0.04);
        gridGfx.strokeRect(x, y, cw, ch);
      }
    }

    // Drop zone — cyan accent border
    const drop = this.add.graphics().setDepth(1);
    drop.fillStyle(0x00f0ff, 0.03);
    drop.fillRect(0, this.ARENA_TOP, W, this.DROP_ZONE_H);
    drop.lineStyle(1, 0x00f0ff, 0.30);
    ...
    // Ambient blobs — Cyberpunk palette (electric blue, magenta, cyan)
    [
      { x: W*0.15, y: H*0.5,  r: 90, c: 0x0033ff, a: 0.07 },
      { x: W*0.85, y: H*0.55, r: 70, c: 0xff00aa, a: 0.06 },
      { x: W*0.5,  y: H*0.3,  r: 60, c: 0x00f0ff, a: 0.04 }
    ].forEach(b => {
    ...
    ['←', '↓', '→'].forEach((arrow, i) => {
      this.add.text(this.LANE_X[i], this.ARENA_TOP + this.DROP_ZONE_H / 2, arrow, {
        fontSize: '18px', color: 'rgba(0,240,255,0.18)', fontFamily: 'Arial'
  ...
  drawBase() {
    ...
    this.baseGfx.lineStyle(2, 0x00aaff, 0.4);
  ...
  updateAim() {
    ...
    for (let y = this.ARENA_TOP + this.DROP_ZONE_H + 5; y < this.GRID_BOT; y += segH + gap) {
      ...
      this.aimGraphic.lineStyle(1.5, 0x00f0ff, alpha);
    ...
    this.aimGraphic.fillStyle(0x00f0ff, 0.7);
  ...
  mergeEffect(x, y, color) {
    ...
    this.showFloatText(x, y - 30, `+${(CREATURES.findIndex(c => c.color === color) + 1) * 10}`, '#f0c040');
  }
    ...
    if (this.baseHP < this.maxBaseHP * 0.2) {
      ...
      if (this._criticalWaves >= 2) {
        this.showFloatText(this.W / 2, this.H / 2,
          `SURVIVAL STREAK: ${this._criticalWaves} WAVES ON THE EDGE!`, '#f0c040');
      }
    } else {
      this._criticalWaves = 0;
    }
  ...
  spawnEnemy(typeIdx, col) {
    ...
    // Rim
    gfx.lineStyle(2, isBoss ? 0xff0033 : cfg.color, 1);
    ...
    const nameT = this.add.text(x, this.ARENA_TOP + 10 - cfg.size, cfg.name, {
      fontSize: '10px', color: isBoss ? '#ff0033' : '#ff6688', fontFamily: 'Arial'
  ...
  _tutDots(step) {
    ...
    for (let i = 0; i < 3; i++) {
      const x = W / 2 + (i - 1) * 18;
      g.fillStyle(i <= step ? 0x00f0ff : 0x334455, 1);
  ...
  _tutShow(step) {
    ...
    if (step === 0) {
      ...
      this._tutCard(16, cardY, W - 32, 116, 0x00f0ff);
      ...
      this.LANE_X.forEach(lx => this._tutArrow(lx, this.ARENA_TOP + this.DROP_ZONE_H * 0.62, 0x00f0ff));
  ...
  _showShop() {
    ...
    // Full-screen dim with violet atmospheric haze
    const dim = this.add.graphics().setDepth(D);
    dim.fillStyle(0x000000, 0.92);
    dim.fillRect(0, 0, W, H);
    for (let i = 0; i < 3; i++) {
      dim.fillStyle(0x4A0090, 0.05 * (1 - i / 3));
      dim.fillEllipse(W / 2, H * 0.32, W * 1.3 * (1 - i * 0.12), 380 * (1 - i * 0.18));
    }
    this._shopObjs.push(dim);

    // Header — hexagonal nameplate (matches BootScene badge language)
    const hdrCX = W / 2, hdrCY = 92;
    const hdrW = W - 30, hdrH = 96, cut = 18;
    const hdrPts = [
      { x: hdrCX - hdrW/2 + cut, y: hdrCY - hdrH/2 },
      { x: hdrCX + hdrW/2 - cut, y: hdrCY - hdrH/2 },
      { x: hdrCX + hdrW/2,       y: hdrCY           },
      { x: hdrCX + hdrW/2 - cut, y: hdrCY + hdrH/2  },
      { x: hdrCX - hdrW/2 + cut, y: hdrCY + hdrH/2  },
      { x: hdrCX - hdrW/2,       y: hdrCY           },
    ];
    const hdrBg = this.add.graphics().setDepth(D + 1).setAlpha(0);
    hdrBg.fillStyle(0x050510, 0.97);
    hdrBg.fillPoints(hdrPts, true);
    hdrBg.lineStyle(1.5, 0x7C3AED, 0.55);
    hdrBg.strokePoints(hdrPts, true);
    const hdrInner = hdrPts.map(p => ({ x: hdrCX + (p.x - hdrCX) * 0.93, y: hdrCY + (p.y - hdrCY) * 0.88 }));
    hdrBg.lineStyle(1, 0x7C3AED, 0.13);
    hdrBg.strokePoints(hdrInner, true);
    this.tweens.add({ targets: hdrBg, alpha: 1, duration: 320 });
    this._shopObjs.push(hdrBg);

    // Violet glow blob behind title
    const hdrGlow = this.add.graphics().setDepth(D + 1).setAlpha(0);
    hdrGlow.fillStyle(0x7C3AED, 0.09);
    hdrGlow.fillEllipse(hdrCX, hdrCY + 4, 320, 80);
    this.tweens.add({ targets: hdrGlow, alpha: 1, duration: 500, delay: 100 });
    this.tweens.add({ targets: hdrGlow, alpha: { from: 0.8, to: 1.6 }, duration: 1800, yoyo: true, repeat: -1, ease: 'Sine.easeInOut', delay: 700 });
    this._shopObjs.push(hdrGlow);

    // "BETWEEN" eyebrow label
    const eyeT = this.add.text(hdrCX, hdrCY - 24, 'B  E  T  W  E  E  N', {
      fontSize: '10px', fontFamily: 'Arial Black', color: '#6B21A8', letterSpacing: 2
    }).setOrigin(0.5).setDepth(D + 2).setAlpha(0);
    this.tweens.add({ targets: eyeT, alpha: 1, duration: 300, delay: 120 });
    this._shopObjs.push(eyeT);

    // "WAVES" dominant — white + violet glow
    const titleT = this.add.text(hdrCX, hdrCY + 8, 'WAVES', {
      fontSize: '40px', fontFamily: 'Arial Black',
      color: '#ffffff', stroke: '#1a0040', strokeThickness: 3,
      shadow: { offsetX: 0, offsetY: 0, color: '#7C3AED', blur: 28, fill: true }
    }).setOrigin(0.5).setDepth(D + 2).setAlpha(0).setScale(0.82);
    this.tweens.add({ targets: titleT, alpha: 1, scaleX: 1, scaleY: 1, duration: 420, delay: 180, ease: 'Back.easeOut' });
    this._shopObjs.push(titleT);

    // "WAVE X COMPLETE" — below nameplate, above coins
    const waveBadgeT = this.add.text(hdrCX, hdrCY + hdrH/2 + 12, `WAVE ${this.wave} COMPLETE`, {
      fontSize: '9px', fontFamily: 'Arial Black', color: '#7C3AED', letterSpacing: 3
    }).setOrigin(0.5).setDepth(D + 2).setAlpha(0);
    this.tweens.add({ targets: waveBadgeT, alpha: 1, duration: 280, delay: 320 });
    this._shopObjs.push(waveBadgeT);

    // Coin display — centered below wave badge
    const coinRowY = hdrCY + hdrH/2 + 30;

    // Create text first to measure width, then center the whole icon+text row
    const coinT = this.add.text(0, 0, `${this.coins} coins available`, {
      fontSize: '14px', fontFamily: 'Arial Black', color: '#f0c040'
    }).setDepth(D + 2);
    const coinIconR = 9, coinGap = 6;
    const totalRowW = coinIconR * 2 + coinGap + coinT.width;
    const rowLeft = W / 2 - totalRowW / 2;
    const cix = rowLeft + coinIconR;
    coinT.setPosition(rowLeft + coinIconR * 2 + coinGap, coinRowY).setOrigin(0, 0.5);
    this._shopObjs.push(coinT);

    const coinGfx = this.add.graphics().setDepth(D + 2);
    coinGfx.fillStyle(0xb07800, 1); coinGfx.fillCircle(cix, coinRowY, coinIconR);
    coinGfx.fillStyle(0xf0c040, 1); coinGfx.fillCircle(cix, coinRowY - 0.5, 7.7);
    coinGfx.lineStyle(1.5, 0xb07800, 0.55); coinGfx.strokeCircle(cix, coinRowY - 0.5, 4.5);
    coinGfx.fillStyle(0xfde68a, 0.75); coinGfx.fillEllipse(cix - 2, coinRowY - 3, 5, 3);
    this._shopObjs.push(coinGfx);

    // FOMO line (suppressed at HP<40%)
    if (this.coins > 80 && this.baseHP < this.maxBaseHP * 0.6 && this.baseHP >= this.maxBaseHP * 0.4) {
      const fomoT = this.add.text(W/2, hdrCY + hdrH/2 + 48, 'Spend before next wave — items shuffle!', {
    ...
    const cardW = 128, cardH = 250, cardY = hdrCY + hdrH/2 + 52;
    const totalW = 3 * cardW + 2 * 8;
    ...
    divLine.lineStyle(1, 0x7C3AED, 0.14);
    ...
    const timerCX = W / 2, timerCY = divY + 42;
    this._shopTimerVal = 10;

    const timerRing = this.add.graphics().setDepth(D + 2);
    const drawRing = (val) => {
      timerRing.clear();
      timerRing.lineStyle(3, 0x1a0040, 1);
      timerRing.strokeCircle(timerCX, timerCY, 22);
      timerRing.lineStyle(3, val <= 3 ? 0xe84040 : 0x7C3AED, 0.9);
      timerRing.beginPath();
      timerRing.arc(timerCX, timerCY, 22, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * (val / 10), false);
    ...
    const timerNumT = this.add.text(timerCX, timerCY, '10', {
      fontSize: '17px', fontFamily: 'Arial Black', color: '#7C3AED'
    }).setOrigin(0.5).setDepth(D + 3);
    this._shopObjs.push(timerNumT);

    const timerLbl = this.add.text(timerCX, timerCY + 32, 'AUTO SKIP', {
      fontSize: '7px', fontFamily: 'Arial Black', color: '#2d1060', letterSpacing: 2
    ...
    // Skip button — ghost violet style
    const skipY = timerCY + 50;
    const skipBg = this.add.graphics().setDepth(D + 1);
    const drawSkipBg = (hover) => {
      skipBg.clear();
      skipBg.fillStyle(hover ? 0x0a0020 : 0x000000, hover ? 0.7 : 0.0);
      skipBg.fillRoundedRect(W / 2 - 54, skipY, 108, 30, 8);
      skipBg.lineStyle(1, hover ? 0x7C3AED : 0x2a1a4a, hover ? 0.9 : 0.6);
      skipBg.strokeRoundedRect(W / 2 - 54, skipY, 108, 30, 8);
    };
    drawSkipBg(false);
    this._shopObjs.push(skipBg);

    const skipBtn = this.add.text(W / 2, skipY + 15, 'SKIP  ›', {
      fontSize: '12px', fontFamily: 'Arial Black', color: '#2a1a4a'
    }).setOrigin(0.5).setDepth(D + 3).setInteractive({ useHandCursor: true });
    skipBtn.on('pointerover', () => { drawSkipBg(true); skipBtn.setColor('#A855F7'); });
    skipBtn.on('pointerout',  () => { drawSkipBg(false); skipBtn.setColor('#2a1a4a'); });
  ...
  _buildShopCard(item, x, y, w, h, D) {
    ...
    const accentCol = isCritical ? 0xe84040 : (canAfford ? 0x7C3AED : 0x180830);

    // Outer ambient glow (affordable — pulsing violet or red)
    if (canAfford) {
      const outerGlow = this.add.graphics().setDepth(D - 1);
      outerGlow.fillStyle(accentCol, 0.07);
      outerGlow.fillRoundedRect(x - 6, y - 6, w + 12, h + 12, 14);
      this.tweens.add({
        targets: outerGlow, alpha: { from: 0.5, to: 1.5 },
        duration: 2200, yoyo: true, repeat: -1, ease: 'Sine.easeInOut'
      });
      this._shopObjs.push(outerGlow);
    }

    // Card background — deep void glass
    const cardBg = this.add.graphics().setDepth(D);
    cardBg.fillStyle(0x06060f, 1);
    cardBg.fillRoundedRect(x, y, w, h, 10);
    cardBg.fillStyle(0xffffff, 0.025);
    cardBg.fillRoundedRect(x + 1, y + 1, w - 2, h / 3, { tl: 10, tr: 10, bl: 0, br: 0 });
    cardBg.lineStyle(isCritical ? 2.5 : 1.5, accentCol, isCritical ? 1.0 : (canAfford ? 0.80 : 0.22));
    cardBg.strokeRoundedRect(x, y, w, h, 10);
    if (canAfford) {
      cardBg.lineStyle(1, accentCol, 0.10);
      cardBg.strokeRoundedRect(x + 3, y + 3, w - 6, h - 6, 8);
    }
    ...
    // Top gradient tint — violet (or red for critical)
    if (canAfford) {
      const tintCol = isCritical ? 0xe84040 : 0x7C3AED;
      const tint = this.add.graphics().setDepth(D);
      tint.fillGradientStyle(tintCol, tintCol, 0x06060f, 0x06060f, isCritical ? 0.18 : 0.12, isCritical ? 0.18 : 0.12, 0, 0);
      tint.fillRoundedRect(x, y, w, 58, { tl: 10, tr: 10, bl: 0, br: 0 });
      this._shopObjs.push(tint);

      // Specular corner glint
      const spec = this.add.graphics().setDepth(D + 1);
      spec.fillStyle(0xffffff, 0.07);
      spec.fillCircle(x + w * 0.80, y + h * 0.09, 20);
      spec.fillStyle(0xffffff, 0.03);
      spec.fillCircle(x + w * 0.82, y + h * 0.07, 30);
      this._shopObjs.push(spec);
    }

    // Icon
    const iconGfx = this.add.graphics().setDepth(D + 1);
    const iconX = x + w / 2, iconY = y + 57;
    this._drawShopIcon(iconGfx, item.id, iconX, iconY, 26, canAfford, isCritical);
    this._shopObjs.push(iconGfx);

    // Name
    const nameT = this.add.text(x + w / 2, y + 97, item.name, {
      fontSize: '12px', fontFamily: 'Arial Black',
      color: canAfford ? '#e2e8f0' : '#2a1a3a',
    ...
    // Description
    const descT = this.add.text(x + w / 2, y + 124, item.desc, {
      fontSize: '10px', fontFamily: 'Arial',
      color: canAfford ? '#7c6fa8' : '#1a0a28',
      align: 'center', wordWrap: { width: w - 12 }
    }).setOrigin(0.5, 0).setDepth(D + 1).setAlpha(0);
    ...
    sep.lineStyle(1, canAfford ? (isCritical ? 0xe84040 : 0x7C3AED) : 0x180830, canAfford ? 0.22 : 0.28);
    sep.beginPath(); sep.moveTo(x + 12, y + 172); sep.lineTo(x + w - 12, y + 172); sep.strokePath();
    this._shopObjs.push(sep);

    // Cost row
    const coinIcon = this.add.graphics().setDepth(D + 1);
    if (canAfford) {
      coinIcon.fillStyle(0xb07800, 1); coinIcon.fillCircle(x + w/2 - 22, y + 186, 8);
      coinIcon.fillStyle(0xf0c040, 1); coinIcon.fillCircle(x + w/2 - 22, y + 185.5, 6.8);
      coinIcon.lineStyle(1.5, 0xb07800, 0.55); coinIcon.strokeCircle(x + w/2 - 22, y + 185.5, 4);
      coinIcon.fillStyle(0xfde68a, 0.75); coinIcon.fillEllipse(x + w/2 - 24, y + 183, 4, 2.5);
    } else {
      coinIcon.fillStyle(0x180830, 1); coinIcon.fillCircle(x + w/2 - 22, y + 186, 8);
    }
    this._shopObjs.push(coinIcon);

    const costT = this.add.text(x + w/2 - 10, y + 186, `${item.cost}`, {
      fontSize: '17px', fontFamily: 'Arial Black',
      color: canAfford ? '#f0c040' : '#180830'
    }).setOrigin(0, 0.5).setDepth(D + 1);
    this._shopObjs.push(costT);

    // BUY button — gold gradient (matches PLAY NOW in BootScene)
    const btnY = y + h - 46;
    const btnW2 = w - 16, btnH2 = 34, btnX = x + 8;
    const btnBg = this.add.graphics().setDepth(D + 1);
    const drawBtnBg = (hover) => {
      btnBg.clear();
      if (canAfford) {
        btnBg.fillGradientStyle(
          hover ? 0xffd700 : 0xe8c040, hover ? 0xffd700 : 0xe8c040,
          hover ? 0x9a6a00 : 0x7a5210, hover ? 0x9a6a00 : 0x7a5210, 1
        );
        btnBg.fillRoundedRect(btnX, btnY, btnW2, btnH2, 8);
        btnBg.lineStyle(1, 0xfff6cc, hover ? 0.6 : 0.42);
        btnBg.beginPath(); btnBg.moveTo(btnX + 14, btnY + 4); btnBg.lineTo(btnX + btnW2 - 14, btnY + 4); btnBg.strokePath();
        btnBg.lineStyle(1.5, 0xc8a951, hover ? 1.0 : 0.85);
        btnBg.strokeRoundedRect(btnX, btnY, btnW2, btnH2, 8);
      } else {
        btnBg.fillStyle(0x050510, 1);
        btnBg.fillRoundedRect(btnX, btnY, btnW2, btnH2, 8);
        btnBg.lineStyle(1, 0x180830, 0.6);
        btnBg.strokeRoundedRect(btnX, btnY, btnW2, btnH2, 8);
      }
    };
    drawBtnBg(false);
    ...
    const btnT = this.add.text(x + w/2, btnY + btnH2/2,
      canAfford ? 'BUY' : 'TOO COSTLY', {
        fontSize: canAfford ? '13px' : '9px',
        fontFamily: 'Arial Black',
        color: canAfford ? '#1a0a00' : '#180830',
        shadow: canAfford ? { offsetX: 0, offsetY: 1, color: '#5a3005', blur: 5, fill: true } : undefined
      }
    ).setOrigin(0.5).setDepth(D + 2);
    this._shopObjs.push(btnT);

    if (canAfford) {
      const hit = this.add.zone(x + w/2, btnY + btnH2/2, btnW2, btnH2)
    ...
    if (isCritical) {
      ...
      this.tweens.add({ targets: critBadge, alpha: 1, duration: 200, delay: 60 });
      this.tweens.add({
  ...
  _drawShopIcon(gfx, id, x, y, r, active, critical = false) {
    ...
    switch (id) {
      ...
      case 'freeze': { // Snowflake
        ...
        gfx.fillCircle(x, y, r + 4);
        gfx.lineStyle(2.5, active ? 0x00f0ff : 0x1a2a2a, 1);
        for (let a = 0; a < 3; a++) {
        ...
        gfx.fillStyle(active ? 0x00f0ff : 0x1a2a2a, 1);
      ...
      case 'armor': { // Shield shape
        ...
        gfx.fillCircle(x, y, r + 4);
        gfx.fillStyle(active ? 0x00aaff : 0x1a1a3a, 1);
        gfx.fillPoints([
          { x: x - r + 4, y: y - r + 4 },
          { x: x + r - 4, y: y - r + 4 },
          { x: x + r - 4, y: y + 2     },
  ...
  _buyItem(item) {
    ...
    switch (item.id) {
      ...
      case 'armor':
        ...
        this.showFloatText(this.W / 2, this.H / 2 - 30, '+30% CREATURE HP!', '#00aaff');
        break;
    }
