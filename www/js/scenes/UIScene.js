...
class UIScene extends Phaser.Scene {
  ...
  drawHeartIcon(gfx, x, y, r) {
    // Soft ambient glow behind heart
    gfx.fillStyle(0xe84040, 0.14);
    gfx.fillCircle(x, y + r * 0.12, r * 1.55);

    // Two upper lobes
    gfx.fillStyle(0xe84040, 1);
    gfx.fillCircle(x - r * 0.5, y - r * 0.15, r * 0.60);
    gfx.fillCircle(x + r * 0.5, y - r * 0.15, r * 0.60);
    // Lower V-point
    gfx.fillTriangle(
      x - r * 1.02, y + r * 0.1,
    ...
    // Inner lighter face (depth)
    gfx.fillStyle(0xff6b6b, 0.45);
    gfx.fillCircle(x - r * 0.42, y - r * 0.22, r * 0.42);
    gfx.fillCircle(x + r * 0.42, y - r * 0.22, r * 0.42);
    // Specular highlight
    gfx.fillStyle(0xffaaaa, 0.55);
    gfx.fillCircle(x - r * 0.3, y - r * 0.28, r * 0.22);
  }

  drawStarIcon(gfx, x, y, r) {
    // Soft glow
    gfx.fillStyle(0xfbbf24, 0.16);
    ...
    // Specular
    gfx.fillStyle(0xfde68a, 0.7);
    gfx.fillCircle(x - r * 0.08, y - r * 0.28, r * 0.24);
  }

  drawCoinIcon(gfx, x, y, r) {
    // Soft glow
    gfx.fillStyle(0xf0c040, 0.16);
    gfx.fillCircle(x, y, r * 1.55);

    // Rim / depth layer
    gfx.fillStyle(0xb07800, 1);
    gfx.fillCircle(x, y, r);
    // Face
    gfx.fillStyle(0xf0c040, 1);
    gfx.fillCircle(x, y - r * 0.06, r * 0.86);
    // Inner embossed ring
    gfx.lineStyle(1.5, 0xb07800, 0.55);
    gfx.strokeCircle(x, y - r * 0.06, r * 0.58);
    // Specular
    gfx.fillStyle(0xfde68a, 0.75);
    gfx.fillEllipse(x - r * 0.24, y - r * 0.32, r * 0.46, r * 0.26);
  }

  // ─── TOP HUD ─────────────────────────────────────────────────────────────────

  drawTopHUD(W) {
    // Void black + violet atmospheric haze
    const bar = this.add.graphics();
    bar.fillStyle(0x000000, 1);
    bar.fillRect(0, 0, W, 54);
    for (let i = 0; i < 4; i++) {
      bar.fillStyle(0x4A0090, 0.09 * (1 - i / 4));
      bar.fillRect(0, 0, W, 54 - i * 10);
    }
    bar.lineStyle(1.5, 0x7C3AED, 0.40);
    ...
    sep.lineStyle(1, 0x7C3AED, 0.18);
    sep.beginPath(); sep.moveTo(W * 0.36, 10); sep.lineTo(W * 0.36, 44); sep.strokePath();
    sep.beginPath(); sep.moveTo(W * 0.68, 10); sep.lineTo(W * 0.68, 44); sep.strokePath();

    // ── HP SECTION ──────────────────────────────────────────────────────────
    // All icons/text share cy=21 — consistent optical row
    const CY = 21;

    const heartGfx = this.add.graphics();
    this.drawHeartIcon(heartGfx, 18, CY, 11);

    this.baseHpText = this.add.text(34, CY, '100%', {
      fontSize: '16px', fontFamily: 'Arial Black', color: '#ff5555',
      shadow: { offsetX: 0, offsetY: 0, color: '#e84040', blur: 10, fill: true }
    }).setOrigin(0, 0.5);

    // Slim HP bar anchored to bottom of section
    ...
    this.drawStarIcon(starGfx, scoreCX - 24, CY, 10);

    this.scoreText = this.add.text(scoreCX + 2, CY, '0', {
      fontSize: '22px', fontFamily: 'Arial Black', color: '#f0c040',
      shadow: { offsetX: 0, offsetY: 0, color: '#7C3AED', blur: 18, fill: true }
    }).setOrigin(0.5, 0.5);

    // WAVE badge — wider violet gradient pill, centered under score
    const waveBg = this.add.graphics();
    waveBg.fillGradientStyle(0x7C3AED, 0x7C3AED, 0x4C1D95, 0x4C1D95, 0.95);
    waveBg.fillRoundedRect(scoreCX - 34, 37, 68, 14, 6);
    waveBg.lineStyle(1, 0xA855F7, 0.55);
    waveBg.strokeRoundedRect(scoreCX - 34, 37, 68, 14, 6);
    this.waveText = this.add.text(scoreCX, 44, 'WAVE 1', {
      fontSize: '7.5px', fontFamily: 'Arial Black', color: '#E9D5FF', letterSpacing: 2
    }).setOrigin(0.5);
    ...
    this.drawCoinIcon(coinGfx, coinCX - 20, CY, 11);

    this.coinsText = this.add.text(coinCX - 4, CY, '0', {
      fontSize: '22px', fontFamily: 'Arial Black', color: '#f0c040',
      shadow: { offsetX: 0, offsetY: 0, color: '#7C3AED', blur: 12, fill: true }
    }).setOrigin(0, 0.5);
  }
  ...
  _drawMiniHpBar(pct) {
    ...
    const sectionW = Math.round(this.W * 0.36);
    const startX = 10;
    const barW = sectionW - 20;
    const y = 38;
    const h = 8;
    const segments = 8;
    const gap = 2;
    ...
    const fillColor = pct > 0.5 ? 0x9B5CF6 : pct > 0.25 ? 0xf97316 : 0xe84040;

    for (let i = 0; i < segments; i++) {
      const sx = startX + i * (segW + gap);
      this.baseHpBar.fillStyle(i < filled ? fillColor : 0x140028, 1);
      this.baseHpBar.fillRoundedRect(sx, y, segW, h, 2);
    }

    // Specular highlight on filled portion
    if (filled > 0) {
      const filledW = filled * (segW + gap) - gap;
      this.baseHpBar.fillStyle(0xffffff, 0.18);
      this.baseHpBar.fillRoundedRect(startX + 2, y + 1, Math.max(0, filledW - 4), 3, 1);
    }
  }
  ...
  drawBottomGuide(W, H) {
    ...
    guide.lineStyle(1, 0xf0c040, 0.12);
    ...
    this.time.delayedCall(400, () => {
      ...
      this.add.text(W / 2, panelY + 70,
        isRecord ? `NEW RECORD: WAVE ${data.wave}!` : `Reached Wave ${data.wave}`,
        {
          ...
          color: isRecord ? '#f0c040' : '#8A8F98'
        }).setOrigin(0.5).setDepth(32);

      this.add.text(W / 2, panelY + 100, `Score: ${data.score.toLocaleString()}`, {
        fontSize: '18px', fontFamily: 'Arial', color: '#f0c040'
      }).setOrigin(0.5).setDepth(32);
      ...
      this.time.delayedCall(500, () => {
        ...
        const adBtn = this.add.text(W / 2, panelY + 183, 'WATCH', {
          fontSize: '11px', fontFamily: 'Arial Black', color: '#f0c040',
        ...
        adBtn.on('pointerout', () => adBtn.setColor('#f0c040'));
      ...
      playBtnGfx.lineStyle(2, 0xf0c040, 0.8);
